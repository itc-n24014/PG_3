...
# 問題集を作成する
for quiz_num in range(int(issue)):
    # 問題集と回答集のファイルを作成
    ...
    # 都道府県の順番をシャッフルする
    profectures = list(capitals.keys())
    random.shuffle(profectures)
    # 47都道府県をループしてそれぞれ問題を作成する
    for question_num in range(len(profectures)):
        # 正答と誤答を抽出する
        correct_answer = capitals[ profectures[question_num] ] # 正解の県庁所在地
        wrong_answers = list(capitals.values()) # 県庁所在地だけをリストに抽出
        del wrong_answers[wrong_answers.index(correct_answer) ] # 正解の県庁所在地だけ除去
        wrong_answers = random.sample(wrong_answers, 3) # 不正解の選択肢を３つだけ抽出
        answer_options = wrong_answers + [correct_answer] # 正解と不正解の選択肢を合わせる(４択になる)
        random.shuffle(answer_options) # 選択肢をシャッフルする
        # 問題文と解答の選択肢を問題ファイルに出力する
        quiz_file.write(f"{question_num+1}. ") # 問題番号
        quiz_file.write(profectures[question_num] ) # 都道府県名
        quiz_file.write("の都道府県庁所在地は？") # 問題文ここまで
        for i in range(4): # ４択の先tなくしを出力
            quiz_file.write(f'{"ABCD"[i]}. {answer_options[i] }\n') # 選択肢をひとつずつ
        quiz_file.write("\n") # 問題文の区切りの空行
        # 答えの選択肢を解答ファイルに出力する
        answer_key_file.write(f"{question_num+1}. ") # 問題番号
        answer_key_file.write("ABCD"[answer_options.index(correct_answer) ] ) # 正解のインデックス番号の選択肢だけ
        answer_key_file.write("\n") # 解答の区切りの改行
    # 47問題の処理が終わったら問題文と解答のファイルをそれぞれクローズする
    quiz_file.close()
    answer_key_file.close()
    # ひとり分の問題作成処理終わり

# 全員分の問題作成終わり
print(f"{issue}人分の問題と解答を作成しました。")